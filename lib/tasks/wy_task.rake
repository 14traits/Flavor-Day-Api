namespace :wy do
  ...
  task load_data: :environment do
    ...
    data["response"]["collection"].each do
      ...
      Location.create!(title: data["response"]["collection"][i]["name"],
                       latitude: data["response"]["collection"][i]["latitude"],
                       longitude: data["response"]["collection"][i]["longitude"],
                       address: data["response"]["collection"][i]["address1"],
                       city: data["response"]["collection"][i]["city"],
                       state: data["response"]["collection"][i]["state"],
                       postal_code: data["response"]["collection"][i]["postalcode"],
                       cl_id: data["response"]["collection"][i]["number"],
                       uid: data["response"]["collection"][i]["uid"])
      i += 1
    end
  end
end
